...
val dxLinguistHome = Paths.get(System.getProperty("user.home")).resolve(".linguist").toFile().apply { mkdirs() }

class Linguist(private val linguistFile: File = dxLinguistHome.resolve("languages.yml")) {
    ...
    init {
        if (!linguistFile.exists()) {
            updateLinguistFile()
        }
    }

    fun readLocalLanguages() = mapper.readValue<Map<String, LanguageDTO>>(linguistFile).map {
        it.key to Language(
            it.key,
            it.value.type,
            it.value.group,
    ...
    fun isOf(path: String, vararg languages: String) = if (languages.isEmpty()) {
        ...
        getLanguages(path).flatMap { it.aliases + it.name + it.group }.any(languages::contains)
    }

    fun getGithubLinguistFile() = URL(githubLinguistFileUrl).readText()

    private fun updateLinguistFile() {
